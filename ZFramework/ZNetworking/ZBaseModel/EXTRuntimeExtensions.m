...
objc_propertyAttributes *objc_copyPropertyAttributes (objc_property_t property) {
    ...
    objc_propertyAttributes *attributes = calloc(1, sizeof(objc_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate objc_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':// copy
            attributes->memoryManagementPolicy = objc_propertyMemoryManagementPolicyCopy;
            break;

        case '&':// retain
            attributes->memoryManagementPolicy = objc_propertyMemoryManagementPolicyRetain;
            break;
        ...
        case 'D':// @dynamic不需要编译器自动生成get/set方法
            attributes->dynamic = YES;
            attributes->ivar = NULL;
            break;

        case 'V':// @synthesize相当于在实现文件声明默认get/set方法
            /*
             ...
             Ti,V_intSynthEquals
             >>>
             attributes->ivar = _intSynthEquals
             **/
