...
static ZDispatchContext *ZDispatchContextCreate(const char *name,
                                                uint32_t queueCount,
                                                NSQualityOfService qos) {
    ZDispatchContext *context = calloc(1, sizeof(ZDispatchContext));
...
static void ZDispatchContextRelease(ZDispatchContext *context) {
    if (!context) return;
    if (context->queues) {
...
static dispatch_queue_t ZDispatchContextGetQueue(ZDispatchContext *context) {
...
static ZDispatchContext *ZDispatchContextGetForQOS(NSQualityOfService qos) {
    static ZDispatchContext *context[5] = {0};
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            ...
            dispatch_once(&onceToken, ^{
                ...
                context[0] = ZDispatchContextCreate("com.zrongl.user-interactive", count, qos);
            });
            return context[0];
        } break;
        case NSQualityOfServiceUserInitiated: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[1] = ZDispatchContextCreate("com.zrongl.user-initiated", count, qos);
            });
            return context[1];
        } break;
        case NSQualityOfServiceUtility: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[2] = ZDispatchContextCreate("com.zrongl.utility", count, qos);
            });
            return context[2];
        } break;
        case NSQualityOfServiceBackground: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[3] = ZDispatchContextCreate("com.zrongl.background", count, qos);
            });
            return context[3];
        } break;
        ...
        default: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[4] = ZDispatchContextCreate("com.zrongl.default", count, qos);
            });
            return context[4];
        } break;
    }
...
@implementation ZDispatchQueuePool
{
@public
    ZDispatchContext *_context;
}

- (void)dealloc {
    if (_context) {
        ZDispatchContextRelease(_context);
        _context = NULL;
    }
}

- (instancetype)initWithContext:(ZDispatchContext *)context {
    self = [super init];
...
- (instancetype)initWithName:(NSString *)name queueCount:(NSUInteger)queueCount qos:(NSQualityOfService)qos {
    ...
    self = [super init];
    _context = ZDispatchContextCreate(name.UTF8String, (uint32_t)queueCount, qos);
...
- (dispatch_queue_t)queue {
    return ZDispatchContextGetQueue(_context);
}

+ (instancetype)defaultPoolForQOS:(NSQualityOfService)qos {
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            static ZDispatchQueuePool *pool;
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[ZDispatchQueuePool alloc] initWithContext:ZDispatchContextGetForQOS(qos)];
            });
            return pool;
        } break;
        case NSQualityOfServiceUserInitiated: {
        ...
        default: {
            ...
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[ZDispatchQueuePool alloc] initWithContext:ZDispatchContextGetForQOS(NSQualityOfServiceDefault)];
            });
            return pool;
        } break;
    }
...
dispatch_queue_t ZDispatchQueueGetForQOS(NSQualityOfService qos) {
    return ZDispatchContextGetQueue(ZDispatchContextGetForQOS(qos));
}
