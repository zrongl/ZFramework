...
@interface LayerDelegate_ : NSObject<CALayerDelegate>
...
@interface CNProgressSlider () {
    CALayer *_lineLayer;
    LayerDelegate_ *_delegate;
}

@end

@implementation CNProgressSlider
...
//        [tap requireGestureRecognizerToFail:pan];
        ...
        _delegate = [[LayerDelegate_ alloc] init];
...
- (void)panAction:(UIPanGestureRecognizer *)panGesture {
    ...
    if (self.sliderDelegate) {
        if ([self.sliderDelegate respondsToSelector:@selector(sliderDidEndDragging:)]
            && panGesture.state ==  UIGestureRecognizerStateEnded) {
            [self.sliderDelegate sliderDidEndDragging:self];
        }else if([self.sliderDelegate respondsToSelector:@selector(sliderDragging:)]
                 && panGesture.state == UIGestureRecognizerStateChanged) {
            [self.sliderDelegate sliderDragging:self];
        }else if ([self.sliderDelegate respondsToSelector:@selector(sliderWillBeginDragging:)]
                  && panGesture.state == UIGestureRecognizerStateBegan){
            [self.sliderDelegate sliderWillBeginDragging:self];
        }
    }
}
...
//    panDistance = panDistance <= (self.frame.size.width - self.sliderDiameter) ? panDistance : (self.frame.size.width - self.sliderDiameter);
//    self.value = panDistance / (self.frame.size.width - self.sliderDiameter);
//    if (self.sliderDelegate && [self.sliderDelegate respondsToSelector:@selector(sliderDragging:)]) {
//        [self.sliderDelegate sliderDragging:self];
//    }
//}
...
- (UIColor *)sliderColor {
    if (!_sliderColor) {
        return [UIColor colorWithRed:214/255.f green:9/255.f blue:18/255.f alpha:1.0f];
    }
    return _sliderColor;
}
...
- (UIColor *)minColor {
    if (!_minColor) {
        return [UIColor colorWithRed:214/255. green:9/255. blue:18/255. alpha:1.0];
    }
    return _minColor;
}
...
- (UIColor *)maxColor {
    if (!_maxColor) {
        return [UIColor colorWithRed:255/255.f green:255/255.f blue:255/255.f alpha:0.2];
...
- (UIColor *)middleColor {
    if (!_middleColor) {
        return  [UIColor colorWithRed:235/255. green:235/255. blue:235/255. alpha:0.7];
    }
    return _middleColor;
}
